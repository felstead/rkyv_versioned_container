...
//!
//! # Error Types
//! Given that introspection of the deserialization errors are more useful in this context (e.g. it
//! is useful to know whether a `version_id` or `type_id` was wrong), we used a type-composed error
//! [RkyvVersionedError] type rather than just `rancor::Error`.  This departs a little from the
//! conventions of `rkyv` but is a little more practical in this scenario where failures might need
//! to be dealt with programmatically.
//!
...
pub fn to_tagged_bytes<T: VersionedContainer>(item: &T) -> Result<AlignedVec, RkyvVersionedError>
where
...
/// "Peeks" at the type_id and version_id inside a tagged byte array generated by [to_tagged_bytes]
/// and returns them.
///
...
/// # Returns
///
/// A `Result` containing the `type_id` and `version_id` of the item, or [RkyvVersionedError] with
/// the `BufferTooSmall` variant if the buffer is undersized.
...
mod tests {
    ...
    fn test_versioned_container() {
        ...
        let twsv_ref = access_from_tagged_bytes::<TestContainer>(&tswv_container_bytes).unwrap();
